...
PLUGINS_BIN = $(BIN_DIR)/plugins

# All source files in our project that must be built into movable object code.
CXXFILES := $(wildcard $(SRC_DIR)/*.cpp $(SRC_DIR)/mvc/*.cpp $(PLUGINS_SRC)/*.cpp)
...
bridge.touch: $(wildcard $(BRIDGE_INCLUDE_DIR)/*) \
	...
	mkdir -p $(OBJ_DIR)/mvc $(DEP_DIR)/mvc
	mkdir -p $(BIN_DIR)/plugins
	echo "include deps.mk" > $@

$(BIN_DIR)/align: $(OBJ_DIR)/main.o \
				  $(OBJ_DIR)/io.o \
				  $(OBJ_DIR)/align.o \
				  $(OBJ_DIR)/align_help.o \
				  $(OBJ_DIR)/mvc/console_views.o \
				  $(OBJ_DIR)/mvc/model.o \
				  $(OBJ_DIR)/mvc/console_controller.o \
				  $(OBJ_DIR)/plugin_utils.o \
				  bridge.touch
	$(CXX) $(CXXFLAGS) $(filter %.o, $^) -o $@ $(LDFLAGS)

build_plugins: $(PLUGINS_BIN)/unsharp.so $(PLUGINS_BIN)/median.so

$(PLUGINS_BIN)/%.so: $(PLUGINS_BIN)/%.o
	$(CXX) -shared -g -o $@ $<
	rm $<

$(PLUGINS_BIN)/%.o: $(PLUGINS_SRC)/%.cpp
	$(CXX) -std=c++14 -fPIC -g -I $(INCLUDE_DIR) -I $(BRIDGE_INCLUDE_DIR) -c -o $@ $<
